...
class Task:
    def __init__(self , taskID , taskTitle="" , description="" , priority=Priority.LOW , status=Status.BACKLOG , createdDT=None , deadlineDT=None):
        ...
        self.createdDT = createdDT if createdDT else datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.deadlineDT = deadlineDT if deadlineDT else (datetime.now() + timedelta(hours=24)).strftime("%Y-%m-%d %H:%M:%S")
        self.Assignees = []
        self.History = {}
        self.comments = {}
